...
from python_mobile_test_template.assist.python import monkey
...
def _by(selector: str):
    ...
    if selector.startswith('«') and selector.endswith('»'):
        return AppiumBy.ANDROID_UIAUTOMATOR, f'new UiSelector().text("{selector[1:-1]}")'

    # --- Handle guessed selectors based on words ---
    if is_word_with_dashes_underscores_or_numbers(
        selector
    ):
        return (
